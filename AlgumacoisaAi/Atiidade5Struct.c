...
int main() {
	...
	while (op != 6) {
		...
		switch (op) {
		case 1: {  //para inserir a data coloque separadamente primeiro o dia depois o sistema vai pedir mes e depois ano, separadamente.
			Data dt;
		...
		case 2: {
			...
			while (continuar == 's' && listaEvt[contEvt].num_part < 10) {
				...
				if (listaEvt[contEvt].num_part < 10) {
					printf("Deseja adicionar outro participante? (s/n): "); //pode escolher se quer adicionar particiantes, mas o limite esta em 10
					scanf(" %c", &continuar);
				} else {
		...
		case 4: {
			char nomeRemover[32];
			printf("Digite o nome do contato que deseja remover: "); //precisa ser igual esta no cadastro para excluir.
			fflush(stdin);
...
/* Utilizei o compilador de C online para dar uma arrumada na identação que tava um pouco atrapalhada
