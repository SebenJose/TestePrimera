...
int main() {
    ...
    // Criar automaticamente um evento de aniversário
    if (contEvt == nevts) {
        nevts *= 2;
        aux2 = (Evento *)realloc(listaEvt, nevts * sizeof(Evento));
        if (aux2 == NULL) {
    ...
    // Configurar o evento de aniversário
    snprintf(listaEvt[contEvt].nome, 20, "Aniversario de %s", listaCts[contCts].nome);
    snprintf(listaEvt[contEvt].desc, 60, "Comemoracao do aniversario de %s", listaCts[contCts].nome);
    listaEvt[contEvt].dt_inicio = listaCts[contCts].nascimento;
    listaEvt[contEvt].dt_fim = listaCts[contCts].nascimento;
    listaEvt[contEvt].num_part = 1;
    listaEvt[contEvt].participantes[0] = listaCts[contCts];

    contEvt++;
    contCts++;

    printf("Contato e evento de aniversario adicionados com sucesso!\n");
    system("PAUSE");
    system("cls");
    break;
}
